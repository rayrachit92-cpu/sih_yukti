  ...
  return <div className="property-3d-root">
    ...
    <div className="floor-selector-3d">
      ...
      <div className="floor-controls">
        <AnimatedButton className={`floor-control-btn ${isolate ? 'active' : ''}`} onClick={() => setIsolate(v => !v)}>{isolate ? 'Show full building' : 'Isolate floor'}</AnimatedButton>
        <AnimatedButton className={`floor-control-btn ${showUnits ? 'active' : ''}`} onClick={() => setShowUnits(v => !v)}>Floor plan {showUnits ? 'ON' : 'OFF'}</AnimatedButton>
      </div>
    </div>
