...
type Street = { id: number; name?: string; highway?: string; coordinates: LatLng[] };
...
function StreetSegment({ a, b }: { a: LocalPoint; b: LocalPoint }) {
  const dx = b.x - a.x;
  const dz = b.z - a.z;
  ...
  return (
    <mesh position={[(a.x + b.x) / 2, 0.035, (a.z + b.z) / 2]} rotation={[0, -angle, 0]} receiveShadow>
      <boxGeometry args={[length, 0.08, width]} />
      <meshStandardMaterial color="#253746" roughness={0.92} metalness={0.05} />
    </mesh>
  );
}

function Streets({ streets, origin }: { streets: Street[]; origin: LatLng }) {
  return <group>
    {streets.map(street => {
      const points = localize(street.coordinates, origin);
      return points.slice(0, -1).map((a, i) => (
        <StreetSegment key={`${street.id}-${i}`} a={a} b={points[i + 1]} />
      ));
    })}
  </group>;
}

function Scene({ parcels, selected, onSelect, streets }: { parcels: ParcelRecord[]; selected?: ParcelRecord; onSelect: (p: ParcelRecord) => void; streets: Street[] }) {
  const origin = useMemo(() => {
    const source = parcels[0]?.coordinates?.[0];
  ...
  return <>
    ...
    <Streets streets={streets} origin={origin} />
...
export default function Full3DTree({ parcels, selected, onSelect, onGenerate }: {
  ...
  const buildings = useMemo(() => parcels.filter(p => p.type === 'Polygon' && p.coordinates.length >= 3), [parcels]);
  const [streets, setStreets] = useState<Street[]>([]);
  const [streetStatus, setStreetStatus] = useState<'loading' | 'ready' | 'offline'>('loading');

  const bbox = useMemo(() => {
    const pts = buildings.flatMap(p => p.coordinates);
    if (!pts.length) return null;
    return {
      minLat: Math.min(...pts.map(p => p.lat)) - 0.001,
      maxLat: Math.max(...pts.map(p => p.lat)) + 0.001,
      minLon: Math.min(...pts.map(p => p.lon)) - 0.001,
      maxLon: Math.max(...pts.map(p => p.lon)) + 0.001,
    };
  }, [buildings]);
  ...
  useEffect(() => {
    ...
    async function loadStreets() {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE}/streets?${qs.toString()}`);
        if (!res.ok) throw new Error(`Street API ${res.status}`);
        ...
        if (!cancelled) { setStreets([]); setStreetStatus('offline'); }
      }
    }
    void loadStreets();
  ...
  return <div className="full-tree-root">
    <Canvas shadows camera={{ position: [130, 110, 130], fov: 42 }} gl={{ antialias: true }}>
      <Scene parcels={buildings} selected={selected} onSelect={onSelect} streets={streets} />
    </Canvas>
    <div className="full-tree-header">
      <div><div className="eyebrow">ADMIN 3D GIS / CITY + PARCEL VIEW</div><h1>Full 3D Map</h1><p>All supplied parcel polygons are shown together with their source-derived building heights and nearby street geometry.</p></div>
      <div className="tree-actions"><AnimatedButton onClick={() => onGenerate(selected)} disabled={!selected}>Create selected 3D ULPIN</AnimatedButton></div>
    </div>
    <div className="full-tree-stats"><span><b>{buildings.length}</b> buildings</span><span><b>{streets.length}</b> streets</span><span><b>{selected?.name ?? 'None'}</b> selected</span><span>Exact parcel footprints</span><span>DEM / DSM heights</span><span>{streetStatus === 'ready' ? 'OSM street context' : streetStatus === 'loading' ? 'Loading streets…' : 'Street API unavailable'}</span></div>
    <div className="full-tree-hint">DRAG ROTATE · SCROLL ZOOM · CLICK ANY BUILDING · DARK ROAD BANDS = OPENSTREETMAP STREET CONTEXT</div>
  </div>;
}
